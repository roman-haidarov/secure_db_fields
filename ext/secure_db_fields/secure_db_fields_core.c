...
const char *sdf_status_name(sdf_status status) {
    switch (status) {
        case SDF_OK: return "ok";
        case SDF_ERR_INVALID_ARGUMENT: return "invalid_argument";
        case SDF_ERR_NO_MEMORY: return "no_memory";
        case SDF_ERR_RANDOM: return "random_failed";
        case SDF_ERR_CRYPTO: return "crypto_failed";
        case SDF_ERR_AUTH: return "authentication_failed";
        case SDF_ERR_FORMAT: return "invalid_envelope";
        case SDF_ERR_KEY: return "key_error";
        case SDF_ERR_UNSUPPORTED: return "unsupported";
        default: return "unknown";
    }
}

void sdf_secure_clear(void *ptr, size_t len) {
    if (ptr && len > 0) OPENSSL_cleanse(ptr, len);
}
...
static sdf_status sdf_validate_key(const unsigned char *key, size_t key_len, char *err, size_t err_len) {
...
sdf_status sdf_encrypt_aes_256_gcm(const unsigned char *plaintext, size_t plaintext_len,
    ...
    *out = NULL;
    *out_len = 0;
    if (sdf_validate_key(key, key_len, err, err_len) != SDF_OK) return SDF_ERR_KEY;
    if (!aad && aad_len > 0) {
...
int sdf_is_valid_envelope(const unsigned char *envelope, size_t envelope_len) {
    if (!envelope || envelope_len < SDF_HEADER_BYTES) return 0;
    if (memcmp(envelope, SDF_MAGIC, SDF_MAGIC_LEN) != 0) return 0;
    if (envelope[4] != SDF_ENVELOPE_VERSION) return 0;
    if (envelope[5] != SDF_ALG_AES_256_GCM) return 0;
    if (sdf_u32be_read(envelope + 6) == 0) return 0;
    return 1;
}
...
int sdf_is_canonical_e164(const unsigned char *value, size_t value_len) {
    size_t i;
    if (!value || value_len < 3 || value_len > 16) return 0;
    if (value[0] != '+') return 0;
...
sdf_status sdf_load_bidx_key(const char *path, const char *domain,
    ...
    for (i = 0; name[i]; i++) {
        if (name[i] >= 'a' && name[i] <= 'z') name[i] = (char)(name[i] - 32);
