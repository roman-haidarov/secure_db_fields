...
native_grep = "SecureDBFields|secure_db_fields|rb_sdf_phone_bidx|sdf_phone_exact_bidx|sdf_is_canonical_e164|sdf_blind_index|HMAC|SHA256|EVP_sha256|OPENSSL|CRYPTO"

raise "PHONE must be canonical E.164" unless SecureDBFields::Phone.canonical_e164?(phone)

preheat_iterations.times do
  out = SecureDBFields::Crypto.phone_blind_index(phone, key: key)
  SDFSample.assert_binary_size!(out, 32, "preheat")
end

SDFSample.print_banner(
  sample_name: sample_name,
  call: "SecureDBFields::Crypto.phone_blind_index(phone, key: key)",
  native_grep: native_grep,
  extra: {
    phone: phone,
    phone_bytes: phone.bytesize,
    preheat_iterations: preheat_iterations
  },
  expected: [
    "SecureDBFields::Crypto.phone_blind_index -> SecureDBFields::Native.phone_blind_index",
    "rb_sdf_phone_bidx",
    "sdf_phone_exact_bidx",
    "sdf_is_canonical_e164 strict validator",
    "sdf_blind_index -> OpenSSL HMAC-SHA256"
  ]
)

last = SDFSample.run_hot_loop do
  SecureDBFields::Crypto.phone_blind_index(phone, key: key)
end
