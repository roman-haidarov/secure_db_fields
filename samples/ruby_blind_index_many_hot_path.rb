...
native_grep = "SecureDBFields|secure_db_fields|rb_sdf_blind_index_many|rb_sdf_hmac_sha256|SHA256_|OPENSSL|CRYPTO"

preheat_iterations.times do
  out = SecureDBFields::Crypto.blind_index_many(values, key: key)
...
SDFSample.print_banner(
  ...
  call: "SecureDBFields::Crypto.blind_index_many(values, key: key)",
  ...
  expected: [
    "SecureDBFields::Crypto.blind_index_many -> SecureDBFields::Native.blind_index_many",
    "rb_sdf_blind_index_many",
    "one Ruby->C call per batch",
...
while SDFSample.monotonic < deadline
  last = SecureDBFields::Crypto.blind_index_many(values, key: key)
  batches += 1
  ops += batch_size
end
